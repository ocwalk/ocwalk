...
object discord extends SprayJsonSupport with LazyLogging {
  ...
  case class DiscordUser(id: String, name: String) {
    ...
    def asUser(implicit config: OcwalkConfig): User = User(id, name, isAdmin)

    /** Returns true if user is one of the admins */
    def isAdmin(implicit config: OcwalkConfig): Boolean = config.discordAdmins.contains(id)
  }
  ...
  def authorize(code: String, config: OcwalkConfig)(implicit s: ActorSystem, m: Materializer, ec: ExecutionContext): Future[Authorization] = for {
    _ <- UnitFuture
    request = FormData(
