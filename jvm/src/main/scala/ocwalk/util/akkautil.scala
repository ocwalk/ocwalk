...
object akkautil {
  ...
  def session()(implicit manager: SessionManagerRef, config: OcwalkConfig): Directive1[Session] = sessionId.flatMap { id =>
  ...
  def adminSession()(implicit manager: SessionManagerRef, config: OcwalkConfig): Directive1[Session] = session().flatMap { session =>
