...
object ml5 extends GlobalContext {
  /** Creates a pitch detector from given model
    ...
    */
  def pitchDetection(modelPath: String, mic: AudioIn): Future[PitchDetection] = {
    val promise = Promise[PitchDetection]()
    ec.execute(() => {
      val pd = lib.facade.ml5.ml5.pitchDetection(modelPath, p5.audioContext, mic.stream, { () => promise.tryFailure(new IllegalStateException(s"Failed to load the model: $modelPath")) })
